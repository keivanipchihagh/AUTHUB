 ...
 * 
 * @author Seyed Mohammad Ghaffarian Edited by Ehsan Edalat
 */
public class FileIO {
...
//        printNioFileInfo("copy.txt");
    ...
    public static void scannerTextFileIO() {
    	try(Scanner input = new Scanner(Paths.get("formatted.txt"))){
    		while(input.hasNext()) {
    			System.out.printf("%s\t%s\t%s\t%f\r\n",
    					input.next(),input.next(),input.next(),input.nextDouble());
    		}
    		System.out.println("Finished reading from formatted.txt");
    	} catch (IOException e) {
			e.printStackTrace();
		}
